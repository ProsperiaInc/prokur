...
const PREFIX = 'CurrencyTextField';
...
const Root = styled('span')((
  ...
  [`& .${classes.textField}`]: (props: any) => ({
    textAlign: props.textAlign || 'right',
  })
}));

/**
 * MuiInputCurrency is a [react](https://reactjs.org/) component with automated currency and number format, and with [Material-ui](https://material-ui.com/) look and feel.
 *
 * MuiInputCurrency is a wrapper component for <a href="https://github.com/autoNumeric/autoNumeric">autonumeric</a> and based on <a href="https://github.com/mkg0/react-numeric">react-numeric</a>.
 *
 * Main features:
...
class MuiInputCurrency extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    const {
      currencySymbol, value, preDefined, ...others
    } = this.props;
    this.autonumeric = new AutoNumeric(this.input, value, {
      ...preDefined,
      ...others,
      onChange: undefined,
      onFocus: undefined,
  ...
  componentWillReceiveProps(newProps) {
    ...
    const isValueChanged = value !== newProps.value && this.getValue() !== newProps.value;
  ...
  callEventHandler(event, eventName) {
    const { [eventName]: propEvent } = this.props;
    if (!propEvent) return;
    propEvent(event, this.getValue());
  }
  ...
  render() {
    ...
    [
      ...
      const { [prop]: innerProp } = this.props;
      otherProps[prop] = innerProp;
    });
